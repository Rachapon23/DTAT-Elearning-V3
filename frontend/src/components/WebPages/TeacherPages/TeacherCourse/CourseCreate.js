...
import { InfoCircleOutlined, EditTwoTone, DeleteTwoTone, BorderOutlined } from '@ant-design/icons';
import { Card, Col, Layout, Row, Tooltip, Button, Input, Typography, Table, Breadcrumb, Steps, Form, Radio, Upload, Image, Select, Calendar, Empty, Tree, Result } from 'antd';
...
const Coursecreate = () => {
    ...
    const onDrop = (info) => {
        ...
        if (info) {
            ...
            setGdata({ ...gData, [`key${dragKey}`]: dropData, [`key${dropKey}`]: dragData })
    ...
    const handleChangeType = (e) => {
        switch(e.target.value) {
            case "Public": setPublicType(true); break;
            case "Private": setPublicType(false); break;
            default: setPublicType(true);
        }
    ...
    const tooltipCourseType = () => {
        return (
            <Tooltip>
                <Row>
                    <Col>
                        {"Public: can study without enroll"}
                    </Col>
                    <Col>
                        {"Private: must enroll before study"}
                    </Col>
                </Row>
            </Tooltip>
        )
        ...
        >
            <Row >
                <Col style={{ width: "100%" }}>
                    ...
                    >
                        <Table dataSource={plantData} columns={plantCol} pagination={{ pageSize: 4, }} size="middle" />
                    </Form.Item>
                        ...
                        <Row align={"middle"} justify={"space-between"}>
                            <Col style={{ width: "45%" }}>
                                <Row justify={"start"}>
                                    ...
                                    <Col style={{ width: "7%", justifyContent: "center", display: "flex" }}>
                                        {<BorderOutlined style={{ backgroundColor: "red", color: "red", fontSize: 20 }} />}
                                    </Col>
                                ...
                                <Row style={{ paddingTop: "1%" }}>
                                    <Col>
                                        Since: 23/03/2023 to 24/03/2023
                                    </Col>
                                </Row>
                            </Col>
                            <Col >
        ...
        >
            <Row >
                ...
                <Radio.Group>
                    ...
                    <Form.Item label="Course Name" required tooltip="This is a required field">
                    ...
                    <Form.Item label="Course Type" required tooltip={tooltipCourseType()}>
                        <Radio.Group defaultValue={courseType} buttonStyle="solid">
                            <Radio.Button value="Public"  onChange={handleChangeType}>Public</Radio.Button>
                            <Radio.Button value="Private" onChange={handleChangeType}>Private</Radio.Button>
                        </Radio.Group>
                    </Form.Item>
                    <Form.Item label="Cover Image">
                        <Row justify={"space-between"}>
                            <Col style={{ width: "100%" }}>
                                <AntdImgCrop showGrid>
                                    <Upload
                                    ...
                                    // onPreview={onPreview}
                                    >
                                        <Button>{fileList.length < 1 ? "Upload" : "Change"}</Button>
    ...
    const pageList = courseType === "Private" ? [courseInformation, timeNroom, managePlant, courseContent, courseCreateFinished]:[courseInformation, courseContent, courseCreateFinished];
    ...
    useEffect(() => {
        handleDisplay()
        renderCardExtarField()
    }, [currentPage, fileList, cardContentList, gData, courseType])
    // Use Effect-End -----------------------------------------------------------------------------------------------------------------------------------------------------------
