...
exports.listCourse = async (req, res) => {
  try {
    const course = await Course.find().populate("teacher room");

    res.json(course);
  } catch (err) {
    console.log(err);
    res.status(500).json({ error: "Unexpected error on list course" });
  }
};
...
exports.removeCourse = async (req, res) => {
  try {
    ...
    await Calendar.findOneAndRemove({ _id: course.calendar });

    const courseRemove = await Course.findOneAndDelete({ _id: req.params.id });
    await Condition.deleteMany({ course: req.params.id });

    res.json(courseRemove);
  } catch (err) {
    console.log(err);
